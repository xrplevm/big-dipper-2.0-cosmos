...
export const formatMinting = (data: Minting, t: TFunction) => [
  {
    key: 'epochIdentifier',
    label: t('epochIdentifier'),
    detail: data.epochIdentifier,
  },
  {
    key: 'reductionFactor',
    label: t('reductionFactor'),
    detail: data.reductionFactor,
  },
  {
    key: 'genesisEpochProvisions',
    label: t('genesisEpochProvisions'),
    detail: data.genesisEpochProvisions,
  },
  {
    key: 'reductionPeriodInEpochs',
    label: t('reductionPeriodInEpochs'),
    detail: `${numeral(data.reductionPeriodInEpochs).format('0.[00]')}`,
  },
  {
    key: 'mintingRewardsDistributionStartEpoch',
    label: t('mintingRewardsDistributionStartEpoch'),
    detail: `${numeral(data.mintingRewardsDistributionStartEpoch).format('0.[00]')}`,
  },
  {
    key: 'mintDenom',
    label: t('mintDenom'),
...
export const formatGov = (data: Gov, t: TFunction) => [
  ...
  },
  {
    ...
    detail: data.expeditedMinDeposit
      ? `${data.expeditedMinDeposit.value} ${data.expeditedMinDeposit.displayDenom.toUpperCase()}`
      : undefined,
  },
  {
    key: 'expeditedThreshold',
    label: t('expeditedThreshold'),
    detail: data.expeditedThreshold
      ? `${numeral(data.expeditedThreshold * 100).format('0.[00]')}%`
      : undefined,
  },
  {
    key: 'expeditedVotingPeriod',
    label: t('expeditedVotingPeriod'),
    detail: data.expeditedVotingPeriod
      ? convertBySeconds(nanoToSeconds(data.expeditedVotingPeriod), t)
      : undefined,
  },
];
